...
const (
	// A metric with no units.
	UnitsNone MetricUnitsType = iota
	// A metric in bytes.
	UnitsBytes
...
// CompositeData represents different kinds of data that needs to be stored.
...
type Sink interface {
	// Stores input CompositeData into the backend.
	Store(*CompositeData) error
...
type Decoder interface {
	// Metrics returns the metrics found in input as a timeseries slice.
	Metrics(input interface{}) ([]Timeseries, error)
}
