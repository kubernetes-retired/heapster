...
func TestInvalidInput(t *testing.T) {
	_, err := NewDecoder().Metrics(struct{}{})
	assert.Error(t, err)
}

func TestEmptyInput(t *testing.T) {
	timeseries, err := NewDecoder().Metrics(source_api.AggregateData{})
...
func TestRealInput(t *testing.T) {
	...
	for index := range statMetrics {
		...
		for innerIndex, entry := range series {
			...
			case "uptime":
				value, ok := entry.Metric.Value.(int64)
				require.True(t, ok)
...
func TestPodLabelsProcessing(t *testing.T) {
	...
	for _, entry := range timeseries {
		assert.Equal(t, expectedLabels, entry.Metric.Labels)
	}
}
