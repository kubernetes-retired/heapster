...
func (sink *kafkaSink) ExportEvents(eventBatch *core.EventBatch) {
	sink.Lock()
	defer sink.Unlock()

	for _, event := range eventBatch.Events {
		var point KafkaSinkPoint
		...
		err := sink.produceKafkaMessage(point, sink.eventTopic)
		if err != nil {
...
func NewKafkaSink(uri *url.URL) (core.EventSink, error) {
