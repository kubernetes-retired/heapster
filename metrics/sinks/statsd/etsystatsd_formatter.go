...
func (formatter *EtsystatsdFormatter) Format(prefix string, name string, labels map[string]string, customizeLabel CustomizeLabel, metricValue core.MetricValue) (res string, err error) {
	...
	var suffix string
	resourceId, ok := labels[core.LabelResourceID.Key]
	if ok {
	...
	if t, ok := labels[core.LabelMetricSetType.Key]; ok {
		switch t {
		...
		default:
			err = fmt.Errorf("Unknown metric set type %s", t)
		}
	} else {
		err = fmt.Errorf("Missing LabelMetricSetType in labels, failed to format metrics")
	}
	return "", err
}
