...
type fakefluentClient struct {
	points []FluentSinkPoint
}

type fakefluentSink struct {
	core.DataSink
	fakeProducer *fakefluentClient
}

func NewFakeFluentClient() *fakefluentClient {
	return &fakefluentClient{[]FluentSinkPoint{}}
}

func (client *fakefluentClient) Name() string {
	return "Apache Fluent Sink"
}

func (client *fakefluentClient) Stop() {
	// nothing needs to be done.
}

func (client *fakefluentClient) Close() error {
	// Do nothing.
	return nil
}

func (client *fakefluentClient) Ping(tag string, record interface{}, options ...fluent.Option) error {
	// Do nothing.
	return nil
}

func (client *fakefluentClient) Shutdown(ctx context.Context) error {
	// Do nothing.
	return nil
}

func (client *fakefluentClient) Post(tag string, msgData interface{}, options ...fluent.Option) error {
	if point, ok := msgData.(FluentSinkPoint); ok {
...
// Returns a fake fluent sink.
func NewFakeSink() fakefluentSink {
	client := NewFakeFluentClient()
	return fakefluentSink{
		&fluentSink{
			Client: client,
			tag:    "test",
		},
