...
func CreateElasticSearchService(uri *url.URL) (*ElasticSearchService, error) {
	...
	if len(opts["nodes"]) > 0 {
		startupFns = append(startupFns, elastic5.SetURL(opts["nodes"]...))
	} else if uri.Opaque != "" {
		startupFns = append(startupFns, elastic5.SetURL(uri.Opaque))
	} else {
	...
	if len(opts["esUserName"]) > 0 && len(opts["esUserSecret"]) > 0 {
		startupFns = append(startupFns, elastic5.SetBasicAuth(opts["esUserName"][0], opts["esUserSecret"][0]))
	}
	...
	if len(opts["maxRetries"]) > 0 {
		...
		startupFns = append(startupFns, elastic5.SetMaxRetries(maxRetries))
	}
	...
	if len(opts["healthCheck"]) > 0 {
		...
		startupFns = append(startupFns, elastic5.SetHealthcheck(healthCheck))
	}
	...
	if len(opts["startupHealthcheckTimeout"]) > 0 {
		...
		startupFns = append(startupFns, elastic5.SetHealthcheckTimeoutStartup(timeout))
	}
	...
	if os.Getenv("AWS_ACCESS_KEY_ID") != "" || os.Getenv("AWS_ACCESS_KEY") != "" ||
		...
		startupFns = append(startupFns, elastic5.SetHttpClient(awsClient), elastic5.SetSniff(false))
	} else {
		if len(opts["sniff"]) > 0 {
			...
			startupFns = append(startupFns, elastic5.SetSniff(sniff))
		}
	}

	esSvc.EsClient, err = elastic5.NewClient(startupFns...)
	if err != nil {
...
func bulkAfterCB(executionId int64, requests []elastic5.BulkableRequest, response *elastic5.BulkResponse, err error) {
	if err != nil {
